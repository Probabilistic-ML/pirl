...
class TensorStandardScaler:
    ...
    def __init__(self, x_dim):
        ...
        self.mu = tf.Variable(self.cached_mu, name="scaler_mu", shape=[1, x_dim], trainable=False)
        self.sigma = tf.Variable(self.cached_sigma, name="scaler_std", shape=[1, x_dim], trainable=False)
...
class FC():
    ...
    def construct_vars(self):
        ...
        # Construct variables
        self.weights = tf.Variable(tf.random.truncated_normal([self.ensemble_size, self.input_dim, self.output_dim],
                                                              stddev=1 / (2 * np.sqrt(self.input_dim)),
                                                              dtype=float_type), name="FC_weights",
                                   shape=[self.ensemble_size, self.input_dim, self.output_dim])
        self.biases = tf.Variable(tf.zeros([self.ensemble_size, 1, self.output_dim], dtype=float_type),
                                  name="FC_biases", shape=[self.ensemble_size, 1, self.output_dim])
...
class BNN(tf.Module):
    ...
    def __init__(self, num_nets=5):
        ...
        Arguments:
            .num_nets (int): (optional) The number of networks in the ensemble. Defaults to 1.
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        self.max_logvar = tf.Variable(np.ones([1, self.layers[-1].get_output_dim() // 2]) / 2., dtype=float_type,
                                      name="max_log_var")
        self.min_logvar = tf.Variable(-np.ones([1, self.layers[-1].get_output_dim() // 2]) * 10., dtype=float_type,
                                      name="min_log_var")
    ...
    def loss(self, inputs, targets):
        train_loss = tf.reduce_sum(self._compile_losses(inputs, targets, inc_var_loss=True))
    ...
    def fit(self, inputs, targets,
        ...
        Arguments:
            ...
            holdout_ratio (float): The ratio of the validation (holdout) set compared to total samples.
            max_logging (int): Ask the original authors, we do not fully understand.
        Returns: None
        """
            ...
            if not hide_progress:
                if holdout_ratio < 1e-12:
                    epoch_range.set_postfix({
                        "Training loss(es)":
                            self.mse_loss(inputs[idxs[:, :max_logging]], targets[idxs[:, :max_logging]])
                    })
                else:
                    epoch_range.set_postfix({
                        "Training loss(es)":
                            self.mse_loss(inputs[idxs[:, :max_logging]], targets[idxs[:, :max_logging]]),
                        "Holdout loss(es)":
                            self.mse_loss(holdout_inputs, holdout_targets)
                    })
    ...
    def predict(self, inputs, factored=False, *args, **kwargs):
        ...
        """
        factored_mean, factored_variance = self.create_prediction_tensors(inputs, factored=True)
        mean = tf.reduce_mean(factored_mean, axis=0)
        var = tf.reduce_mean(factored_variance, axis=0) + tf.reduce_mean(tf.square(factored_mean - mean), axis=0)
...
def make_bnn(n_inps, n_outs, widths, activations, weight_decays, learning_rate):
    assert len(widths), "At least one layer needs to be defined."
    assert len(widths) == len(activations) < len(weight_decays), "Configuration mismatch!"
    model = BNN()
    model.add(FC(widths[0], input_dim=n_inps, activation=activations[0], weight_decay=weight_decays[0]))

    for width, act, wd in zip(widths[1:], activations[1:], weight_decays[1:]):
        model.add(FC(width, activation=act, weight_decay=wd))

    model.add(FC(n_outs, weight_decay=weight_decays[-1]))
    model.finalize(tf.optimizers.Adam, {"learning_rate": learning_rate})
    return model
